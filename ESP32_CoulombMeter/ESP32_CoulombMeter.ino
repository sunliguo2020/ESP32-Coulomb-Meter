...
void setup() {
  ...
  setupHardware();
  setupDisplay();
  setConfigModeRefs(&server, &dnsServer, &tft);
  setConfigModePreferences(&preferences);
  loadCalibration();
  loadBlinkerAuth();
...
void setupHardware() {
  ...
  if (!ina226.begin()) {
    Serial.println("❌ INA226 初始化失败！请检查I2C连接或确认设备地址");
    Serial.println("   建议：确认 INA226 连接到 21/22 引脚，且地址为 0x40~0x4F");
  } else {
